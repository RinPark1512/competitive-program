...
public class rectangleSurround {

    public static void main(String[] args) throws IOException {
        // Fast Reader class provided from class, see below
        FastReader fr = new FastReader();
        PrintWriter pw = new PrintWriter(System.out);

//        track how many rectangles to read and put into the graph
        int numRect = 0;
        numRect=fr.nextInt();
        while (numRect != 0){
            // create a "graph", upper bound 500 (max input value)
            boolean[][] graph = new boolean[500][500];

            for (int i = 0; i < numRect; i++){
                // rectangle coordinates are read a, b, c, d
                // correspond to (a,b) and (c, d)
                int a = fr.nextInt();
                int b = fr.nextInt();
                int c = fr.nextInt();
                int d = fr.nextInt();
                // for every square/grid the rectangle occupies on the graph, mark as "true"
                for (int j = a; j < c; j++){
                    for (int k = b; k < d; k++){
                        graph[j][k] = true;
                    }
            ...
            long sum = 0;
            // iterate through the entire graph, count how many squares/grids are marked
            for (int i = 0; i < 500; i++){
                for (int j = 0; j < 500; j++){
                    sum++;
                }
